                    ...
                    <tbody>
                        <template v-for="(doc, index) in reportData" :key="index">
                            ...
                            <tr> 
                                ...
                                <th >{{ doc.totalDiscount }}</th>
                                <th >{{ doc.totalReceive }}</th>       
                            </tr>
                        </template>
                    </tbody>
                </table>
                <span class="float-right text-lg font-medium mt-2">
                    Total: {{ grandTotalReceive }}
                </span>
            </div>
        </ReportLayoutVue>
    </div>
...
const reportName = ref('Report Receipt Summary By Employee')
const columns = ref([
...
const submit = () => {
    ...
    reportData.value = []
    Meteor.call('ReceiptSummaryByEmp', { ...selector }, (err, res) => {
        if (err) {
            console.log(err)
        } else {
            reportData.value = res.datares
            grandTotalReceive.value = res.grandTotalReceive
