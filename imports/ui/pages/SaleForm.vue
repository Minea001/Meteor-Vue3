        ...
        <div class="row q-col-gutter-x-xl q-col-gutter-y-md">
          <div class="col-xs-12 col-md-12 col-lg-12">
            <div class="row q-col-gutter-y-sm">
              ...
              <div class="col-4" style="padding-right: 10px;">
                <q-select dense v-model="form.customerId" :options="showCustomer" outlined emit-value map-options
                  option-value="_id" option-label="name" label="Select Customer Name" />
              </div>
              <div class="col-4" style="padding-right: 10px;">
                <q-select dense v-model="form.employeeId" outlined :options="showEmployee" emit-value map-options
                  option-value="_id" option-label="name" label="Select Employee Name" />
              </div>
        ...
        <tbody>
          ...
          <tr v-for="(it, index) in list_form" :key="index" >
            ...
            <td class="text-center">
              <q-select dense v-model="it.itemId" :options="showItem" outlined emit-value map-options option-value="_id"
                option-label="name" @update:model-value="changerow(it,'Item')" >
              </q-select>
                <!-- 'Item' is when select and it was type item -->
            </td>

            <td class="text-center">
              <q-input dense v-model.number="it.qty" outlined @update:model-value="changerow(it)"></q-input>
              
            </td>
              
            <td >
              <div class="text-center">
              <q-input dense v-model.number="it.price"  outlined  @update:model-value="changerow(it)"></q-input>
            ...
            <td >
              <div class="text-center">
              <q-text dense v-model="it.amount"  outlined disable >{{ it.amount }}</q-text>
            </div>
           </td>
            <td class="text-center">
              <q-btn dense color="primary" v-if="index==(list_form.length-1)"  @click="add" style="font-weight: bold; margin-right: 5px;">Add</q-btn>
              <q-btn dense color="red"  @click="add" style="font-weight: bold; margin-right: 5px;">Del</q-btn>
            </td>
          </tr>
        </tbody>
      </q-markup-table>
...
watch(()=>list_form.value, (newValue) => {
