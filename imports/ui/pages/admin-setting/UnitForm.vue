...
const checkExist = (selector) => {
    return new Promise((resolve, reject) => {
        Meteor.call('checkunitExist', { selector }, (err, res) => {
            if (err) {
                reject(err)
                console.log(err)
            } else {
...
const insert = () => {
    Meteor.call('Unitadd', form.value, (err, res) => {
        if (err) {
            console.log(err)
...
const update = () => {
    Meteor.call('updateUnit', form.value, (err, res) => {
        if (err) {
            Notify.error({ message: err.reason || err })
        } else {
...
const remove=() => {
    $q.dialog({
        ...
        Meteor.call('removeUnit', { id: props.showId }, (err, res) => {
            if (err) {
                Notify.error({ message: err.reason || err })
            } else {
...
watch(
    ...
    (value) => {
        if (value) {
            Meteor.call('GetUnitById', value, (err, res) => {
                form.value = res
            })
        }
    }
